        ...
        <div className="flex flex-wrap items-end gap-8">
          {(["cyan", "pink", "green"] as const).map((c) => (
            <div key={c} className="flex flex-col gap-2 items-center">
              ...
              <span className="text-xs font-orbitron text-white/60 uppercase">
                {c}
              </span>
            </div>
          ))}
        ...
        <div className="flex flex-wrap items-end gap-8">
          {([60, 100, 160, 220] as const).map((w) => (
            <div key={w} className="flex flex-col gap-2 items-center">
              ...
              <span className="text-xs font-orbitron text-white/60">{w}px</span>
            </div>
          ))}
        </div>
      </div>
          ...
          {(["monotone", "linear", "step"] as const).map((c) => (
            <div key={c} className="flex flex-col gap-2 items-center">
              <NeonSparkline
                ...
                curve={c}
              />
              <span className="text-xs font-orbitron text-white/60">{c}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-4">
          {[
            ...
            >
              <p className="font-orbitron text-[10px] text-white/65 uppercase tracking-widest mb-1">
                {item.label}
              </p>
              <p className="font-orbitron text-xl text-white font-bold mb-1">
                {item.value}
              </p>
              <p
                className={`text-xs font-mono mb-3 ${item.color === "green" ? "text-neon-green" : item.color === "pink" ? "text-neon-pink" : "text-neon-cyan"}`}
              >
                {item.trend}
              </p>
              <div className="-mx-4">
