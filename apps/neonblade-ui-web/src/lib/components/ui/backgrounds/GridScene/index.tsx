...
export function GridScene({
  ...
  useEffect(() => {
     ...
     * @param edgeY      - The pixel Y where the plane's near edge starts (floor bottom-of-gap, ceiling top-of-gap)
     * @param vanishY    - The pixel Y where perspective lines converge (horizon centre)
    ...
    const drawPlane = (edgeY: number, vanishY: number, isFloor: boolean) => {
      ...
      // are fully opaque — the opposite of the old behaviour.
      ...
      for (let i = 0; i < maxDepth + 1; i++) {
        ...
        const fadeIn = Math.min(normalised / 0.35, 1); // fully opaque after 35% depth
